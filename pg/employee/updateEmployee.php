...
<html lang="en">
               ...
               <div class="row">
                  <div class="col-xl-12">
                    <section class="hk-sec-wrapper">
                           <h4 class="hk-sec-title">사원 정보변경</h4>
                           <p class="mb-40">Update Employee</p>
                           <div class="row">
                               <div class="col-sm">
                                  <form method="POST" action="/form/form_employee/updateEmployee.php?id=<?php echo $_GET['id']?>" enctype="multipart/form-data">
   ...
   <script>
      ...
      <?php
        ...
        $api = new getForm();
        $employees = $api -> select_employee($_GET['id']);
        while ($row = $employees->fetch(PDO::FETCH_BOTH)){?>
          $("input[name=accountCode]").val("<?php echo $row['account_id']?>");
          $("input[name=account_name]").val("<?php echo $row['account_name']?>" +" / " + "<?php echo $row['account_code']?>");
          $("input[name=employee_code]").val("<?php echo $row['code']?>");
          $("input[name=employee_name]").val("<?php echo $row['name']?>");
      ...
      // 등록된 회사정보 조회
      <?php
        $accounts = $api -> select_accounts();
        while ($row = $accounts->fetch(PDO::FETCH_BOTH)){?>
