...
<html lang="en">
               ...
               <div class="row">
                  <div class="col-xl-12">
                    <section class="hk-sec-wrapper">
                           <h4 class="hk-sec-title">거래처 정보변경</h4>
                           <p class="mb-40">Update Account</p>
                           <div class="row">
                               <div class="col-sm">
                                  <form method="POST" action="/form/form_account/updateAccount.php?id=<?php echo $_GET['id']?>" enctype="multipart/form-data">
                                       <div class="form-row">
   ...
   <script>
     <?php
       ...
       $api = new getForm();
       $orders = $api -> select_account($_GET['id']);
       while ($row = $orders->fetch(PDO::FETCH_BOTH)){?>
         $("input[name=name]").val("<?php echo $row['name']?>");
         $("input[name=code]").val("<?php echo $row['code']?>");
         $("input[name=tel]").val("<?php echo $row['tel']?>");
         $("input[name=product]").val("<?php echo $row['products']?>");
         <?php
       }
     ?>
   </script>
